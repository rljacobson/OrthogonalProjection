...
// Returns a number in (−pi, pi]. float OrthoToRectLambda(float x, float y){ |x|, |y| <= 1. return
// atan2f(-y, x); }
...
// map_x
cv::Mat map_lambda(static_cast<int>(diameter), static_cast<int>(diameter), CV_32FC1);
// map_y
cv::Mat map_phi(static_cast<int>(diameter), static_cast<int>(diameter), CV_32FC1);
...
void initMaps(float inwidth= 3840.0f, float inheight= 1920.0f, float indiam = 0.0f){
    ...
    map_lambda = cv::Mat(static_cast<int>(diameter), static_cast<int>(diameter), CV_32FC1);
    map_phi = cv::Mat(static_cast<int>(diameter), static_cast<int>(diameter), CV_32FC1);
    mask = cv::Mat(static_cast<int>(diameter), static_cast<int>(diameter), CV_8UC3);

    // map_x
    map_lambda.forEach<float>([](float &x_coord, const int *pos) -> void {
        // Scale to [-1,1]X[-1,1]
        float x = pos[0]/diameter*2.0f - 1.0f;
        float y = pos[1]/diameter*2.0f - 1.0f;
        // Spread out region near sand.
        x_coord = (OrthoToRectLambda(x, y)/M_PI + 1.0f)*0.5f*width;
    });
    // map_y
    map_phi.forEach<float>([](float &y_coord, const int *pos) -> void {
        // Scale to [-1,1]X[-1,1]
        float x = pos[0]/diameter*2.0f - 1.0f;
        float y = pos[1]/diameter*2.0f - 1.0f;
        // Spread out region above equator.
        y_coord = height*(1.0f - speedup((OrthoToRectPhi(x*.8, y*.8)/M_PI + 0.5f)));
    });
...
void remap(cv::Mat &in, cv::Mat &out, int diam) {
    ...
    cv::remap(in, out, map_lambda, map_phi, cv::INTER_LANCZOS4, cv::BORDER_WRAP, cv::Scalar(0, 0, 0));
